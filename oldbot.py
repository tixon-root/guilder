...
# ==================== /botset КОМАНДЫ ====================
# Используй прямо в нужном чате/теме:
#   /botset admin  — этот чат станет чатом заявок
#   /botset guild  — этот чат станет чатом гильдии
#   /botset        — показать текущие настройки

@router.message(Command("botset"))
async def cmd_botset(message: Message):
    ...
    topic_id   = message.message_thread_id  # None если без темы
    topic_info = f" → тема <code>{topic_id}</code>" if topic_id else ""

    # Показать текущие настройки
    if not arg or arg == "info":
        s = await get_settings()
        a_id  = s.get("admin_chat_id",  "не настроен")
        a_top = s.get("admin_topic_id", "нет")
        g_id  = s.get("guild_chat_id",  "не настроен")
        g_top = s.get("guild_topic_id", "нет")
        await message.answer(
            "ℹ️ <b>Текущие настройки бота</b>\n\n"
            f"📬 Чат заявок: <code>{a_id}</code> (тема: <code>{a_top}</code>)\n"
            f"📢 Чат гильдии: <code>{g_id}</code> (тема: <code>{g_top}</code>)\n\n"
            "Команды:\n"
            "<code>/botset admin</code> — назначить этот чат для заявок\n"
            "<code>/botset guild</code> — назначить этот чат для гильдии"
        )
        return

    # Назначить чат заявок
    if arg == "admin":
        await save_settings({
        ...
        await message.answer(
            f"✅ <b>Чат заявок настроен!</b>\n\n"
            f"📍 {chat_title}{topic_info}\n"
            f"ID: <code>{chat_id}</code>\n\n"
            "Сюда будут приходить новые заявки на вступление."
        )
        return

    # Назначить чат гильдии
    if arg == "guild":
        await save_settings({
        ...
        await message.answer(
            f"✅ <b>Чат гильдии настроен!</b>\n\n"
            f"📍 {chat_title}{topic_info}\n"
            f"ID: <code>{chat_id}</code>\n\n"
            "Сюда будут приходить уведомления о новых участниках и неактивных игроках."
        )
        return
    ...
    await message.answer(
        "❌ Неверный аргумент.\n\n"
        "Используй:\n"
        "<code>/botset admin</code> — чат для заявок\n"
        "<code>/botset guild</code> — чат для гильдии\n"
        "<code>/botset</code> — текущие настройки"
    )
